...
*/
	...
	I2C1->CCR = 80<<0;  // check calculation in PDF

	// Configure the rise time register
	I2C1->TRISE = 17;  // check PDF again
...
uint8_t MPU_FindAdress(uint8_t Address){
	uint8_t addressMatch;
	uint8_t buffer[10];

	I2C_Start ();
	addressMatch = I2C_FindAddress(Address);
//	I2C_Write (0x00);
//	I2C_Start ();  // repeated start
//	I2C_Read (Address+0x01, buffer, 1);
//	I2C_Stop ();
...
uint8_t I2C_FindAddress (uint8_t Address)
{
	...
	}while (!(I2C1->SR1 & I2C_SR1_ADDR) && !timeout);  // wait for ADDR bit to set
